...
class AMASSSubsetDataset(Dataset):
    ...
    def _apply_bone_length_perturbations(self, sequence: np.ndarray, parents: np.ndarray, children_map: dict, noise_scale: float) -> np.ndarray:
        ...
        for t_idx in range(T):
            current_frame_original = sequence[t_idx].copy()
            current_frame_perturbed = perturbed_sequence[t_idx]

            for joint_idx in processing_order:
                parent_idx = parents[joint_idx]
                if parent_idx == -1: continue

                parent_pos_perturbed = current_frame_perturbed[parent_idx, :]
                original_parent_pos = current_frame_original[parent_idx, :]
                original_child_pos = current_frame_original[joint_idx, :]
    ...
    def _create_windows(self):
        ...
        print_seq_interval = 50
        print_window_interval = 10000
...
if __name__ == '__main__':
    ...
    num_source_joints = 24
    num_target_joints = 24
    seq_len = 200
    
    dummy_poses = np.random.rand(seq_len, num_source_joints, 3).astype(np.float32) * 2 - 1
