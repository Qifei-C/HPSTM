# src/models/pose_refiner_transformer_old.py
import torch
...
class ManifoldRefinementTransformerOld(nn.Module):
    ...
    def forward(self, noisy_r3j_seq):
        ...
        # Transformer Decoder
        tgt_queries = self.decoder_query_embed.weight.unsqueeze(1).repeat(1, batch_size, 1) # (S, B, d_model)
        seq_first_tgt_queries = self.pos_encoder(tgt_queries) # (S, B, d_model)
